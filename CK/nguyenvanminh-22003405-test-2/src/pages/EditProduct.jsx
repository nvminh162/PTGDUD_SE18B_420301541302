...
const EditProduct = () => {
  ...
  // Sử dụng custom hook useFetch để lấy thông tin sản phẩm cần chỉnh sửa
  const { data: product, loading: fetchLoading, error: fetchError } = useFetch(`http://localhost:3000/products/${id}`);
  ...
  // Cập nhật formData khi có dữ liệu sản phẩm
  useEffect(() => {
    if (product) {
      setFormData({
        name: product.name,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      // Chuyển đổi giá từ string sang number và cập nhật sản phẩm
      const productData = {
        ...formData,
        id: Number(id),
